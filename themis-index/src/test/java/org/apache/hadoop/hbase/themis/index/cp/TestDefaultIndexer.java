...
public class TestDefaultIndexer extends IndexTestBase {
  ...
  public void testLoadSecondaryIndexesForTable() throws IOException {
    Map<IndexColumn, TableName> columnIndexes = new HashMap<>();
    DefaultIndexer indexer = new DefaultIndexer(conf);
    indexer.loadSecondaryIndexesForTable(admin.getDescriptor(MAIN_TABLE), columnIndexes);
    assertEquals(1, columnIndexes.size());
    IndexColumn indexColumn = new IndexColumn(MAIN_TABLE, INDEX_FAMILY, INDEX_QUALIFIER);
    assertTrue(columnIndexes.containsKey(indexColumn));
    assertEquals(INDEX_TABLE, columnIndexes.get(indexColumn));
  }
  ...
  public void testAddIndexMutations() throws IOException {
    ...
    mutationCache.addMutation(MAIN_TABLE,
      CellBuilderFactory.create(CellBuilderType.SHALLOW_COPY).setRow(ROW).setFamily(ANOTHER_FAMILY)
      .setQualifier(INDEX_QUALIFIER).setTimestamp(Long.MAX_VALUE).setType(Type.Put).setValue(VALUE)
      .build());
    mutationCache.addMutation(MAIN_TABLE,
      CellBuilderFactory.create(CellBuilderType.SHALLOW_COPY).setRow(ROW).setFamily(INDEX_FAMILY)
      .setQualifier(ANOTHER_QUALIFIER).setTimestamp(Long.MAX_VALUE).setType(Type.Put).setValue(VALUE)
      .build());
    mutationCache.addMutation(MAIN_TABLE,
      CellBuilderFactory.create(CellBuilderType.SHALLOW_COPY).setRow(ROW).setFamily(INDEX_FAMILY)
      .setQualifier(INDEX_QUALIFIER).setTimestamp(Long.MAX_VALUE).setType(Type.DeleteColumn).setValue(VALUE)
      .build());
    DefaultIndexer indexer = new DefaultIndexer(conf);
    indexer.addIndexMutations(mutationCache);
    assertEquals(4, mutationCache.size());
    ...
    mutationCache.addMutation(MAIN_TABLE,
      ...
      .build());
    indexer.addIndexMutations(mutationCache);
    assertEquals(5, mutationCache.size());
    ...
    assertEquals(Type.Put, typeAndValue.getFirst());
    assertArrayEquals(HConstants.EMPTY_BYTE_ARRAY, typeAndValue.getSecond());
  }
  ...
  protected void checkReadResults(byte[][] values, ColumnCoordinate[] columns, Result result) {
    ...
    for (int i = 0; i < values.length; ++i) {
      ...
      assertArrayEquals(values[i], result.getValue(column.getFamily(), column.getQualifier()));
    }
  }
  ...
  public void testWriteTransactionWithIndex() throws IOException {
    ...
    nextTsAndCreateTransaction();
    Result result =
      transaction.get(MAIN_TABLE, new ThemisGet(ROW).addColumn(INDEX_FAMILY, INDEX_QUALIFIER));
    checkReadResult(VALUE, COLUMN, result);
    result = transaction.get(INDEX_TABLE, new ThemisGet(VALUE)
      .addColumn(IndexMasterObserver.THEMIS_SECONDARY_INDEX_TABLE_FAMILY_BYTES, ROW));
    assertEquals(1, result.size());
    assertArrayEquals(HConstants.EMPTY_BYTE_ARRAY, CellUtil.cloneValue(result.rawCells()[0]));
  ...
  public void testIndexGet() throws IOException {
    ...
    ResultScanner scanner =
      transaction.getScanner(MAIN_TABLE, new ThemisScan().addColumn(FAMILY, QUALIFIER));
    assertFalse(scanner instanceof IndexScanner);
    checkReadResult(VALUE, COLUMN, scanner.next());
    scanner.close();

    // index get column
    IndexGet indexGet =
      new IndexGet(INDEX_COLUMN, VALUE, new DataGet().addColumn(FAMILY, QUALIFIER));
    scanner = transaction.getScanner(MAIN_TABLE, indexGet);
    assertTrue(scanner instanceof IndexScanner);
    checkReadResult(VALUE, COLUMN, scanner.next());
    ...
    scanner = transaction.getScanner(MAIN_TABLE, indexGet);
    checkReadResults(new byte[][] { VALUE, ANOTHER_VALUE },
      new ColumnCoordinate[] { COLUMN, COLUMN_WITH_ANOTHER_FAMILY }, scanner.next());
    checkAndCloseScanner(scanner);
    ...
    nextTsAndCreateTransaction();
    indexGet = new IndexGet(INDEX_COLUMN, ANOTHER_VALUE,
      new DataGet().addColumn(FAMILY, QUALIFIER).addFamily(ANOTHER_FAMILY));
    scanner = transaction.getScanner(MAIN_TABLE, indexGet);
    assertTrue(scanner instanceof IndexScanner);
    checkReadResults(new byte[][] { ANOTHER_VALUE, ANOTHER_VALUE },
      new ColumnCoordinate[] { COLUMN, COLUMN_WITH_ANOTHER_FAMILY }, scanner.next());
    checkAndCloseScanner(scanner);
    ...
    indexGet =
      new IndexGet(INDEX_COLUMN, ANOTHER_VALUE, new DataGet().addColumn(FAMILY, QUALIFIER));
    scanner = transaction.getScanner(MAIN_TABLE, indexGet);
    assertTrue(scanner instanceof IndexScanner);
    ...
    assertTrue(scanner instanceof IndexScanner);
    result = scanner.next();
    assertArrayEquals(ANOTHER_ROW, result.getRow());
    checkReadResult(VALUE, COLUMN, result);
    result = scanner.next();
  ...
  public void testIndexScan() throws IOException {
    ...
    assertTrue(scanner instanceof IndexScanner);
    checkReadResult(ANOTHER_VALUE, COLUMN, scanner.next());
    checkReadResults(new byte[][] { VALUE, VALUE, VALUE },
      new ColumnCoordinate[] { COLUMN, COLUMN_WITH_ANOTHER_QUALIFIER, COLUMN_WITH_ANOTHER_FAMILY },
      scanner.next());
    checkAndCloseScanner(scanner);
