...
public class TestBaseTimestampOracle {
    ...
    for (int i = 0; i < threadCounts.length; ++i) {
      ...
      for (GetTimestampInvokeThread getThread : getThreads) {
        ...
        for (int j = 0; j < results.length; ++j) {
          assertTrue(results[j] instanceof Long);
      ...
      assertEquals(expectDistinctTsCount, tsSet.size());
    }
  }

  private static final String TIMESTAMP_EXCEPTION_STRING = "Timestamp Exception";

  protected void createBatchOracleWithTimeoutImpl(int queueSize, int requestTimeout,
      int implTimeout) throws IOException {
  ...
  public void testGetTimestampWithBatchImplTimeout() throws Exception {
    ...
    try {
      ...
      assertTrue(consume >= 100 && consume < 150);
    }
    ...
    waitGetThreads();
    assertTrue(getRootCause(getThreads[0].results[0]) instanceof TimeoutException);
    assertTrue(getRootCause(getThreads[1].results[0]) instanceof TimeoutException);
    assertTrue(getRootCause(getThreads[2].results[0]) instanceof RejectedExecutionException);
  }

  @Test
  public void testTwoRequestsOfTheSameThreadMustGetTimestampFromTheDifferentBatchRequest()
      throws Exception {
    ...
    assertTrue(getRootCause(threadB.results[0]) instanceof TimeoutException);
    threadA.run();
  ...
  public void testGetTimestampWithBatchImplException() throws Exception {
    ...
    for (GetTimestampInvokeThread thread : getThreads) {
      for (Object result : thread.results) {
        ...
        assertTrue(rootException instanceof IOException);
        assertEquals(TIMESTAMP_EXCEPTION_STRING, rootException.getMessage());
      }
    }
  }
  ...
  public void testThemisTimestamp() throws Exception {
    ...
    assertEquals(100, pTimestamp.getStartTs());
    try {
