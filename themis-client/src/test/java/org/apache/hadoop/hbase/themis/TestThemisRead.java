 ...
 /*
  ...
  public void testSetFilter() throws IOException {
    ...
    for (ThemisRead reader : readers) {
      ...
      assertEquals(PrefixFilter.class.getName(), reader.getFilter().getClass().getName());
      filterList = new FilterList();
      filterList.addFilter(new PrefixFilter());
      reader.setFilter(filterList);
      assertEquals(FilterList.class.getName(), reader.getFilter().getClass().getName());
    }
  }*/
  ...
  public void testCheckContainingPreservedColumns() {
    ...
    familyMap.get(FAMILY)
      .add(ColumnUtil.getPutColumn(new Column(FAMILY, QUALIFIER)).getQualifier());
    try {
      ThemisRead.checkContainingPreservedColumns(familyMap);
      fail();
    } catch (IOException e) {
