...
public abstract class WorkerRegister extends Configured {
  ...
  public static WorkerRegister getWorkerRegister(Configuration conf) throws IOException {
    ...
    if (register == null) {
      synchronized (registerLock) {
        if (register == null) {
          try {
            register = (WorkerRegister) (Class.forName(workerRegisterCls)
              .getConstructor(Configuration.class).newInstance(conf));
          } catch (Exception e) {
            LOG.error("create workerRegister fail", e);
  ...
  public static class NullWorkerRegister extends WorkerRegister {
    private static final Logger LOG = LoggerFactory.getLogger(NullWorkerRegister.class);
