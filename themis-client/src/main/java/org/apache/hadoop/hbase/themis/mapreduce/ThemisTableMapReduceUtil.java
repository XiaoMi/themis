...
public class ThemisTableMapReduceUtil {
  ...
  public static void addDependencyJars(Job job) throws IOException {
    ...
    TableMapReduceUtil.addDependencyJarsForClasses(job.getConfiguration(),
      ThemisEndpointClient.class);
  }

  public static void initTableMapperJob(List<Scan> scans, Class<? extends TableMapper> mapper,
      Class<? extends WritableComparable> outputKeyClass, Class<?> outputValueClass, Job job)
      throws IOException {
  ...
  // for reduce
  public static void initMultiTableReducerJob(Class<? extends Reducer> reducer, Job job)
      throws IOException {
  ...
  public static void initTableReducerJob(String table, Class<? extends Reducer> reducer, Job job,
      Class partitioner, String quorumAddress, String serverClass, String serverImpl,
      boolean addDependencyJars, Class<? extends OutputFormat> outputFormatClass)
      throws IOException {
    Configuration conf = job.getConfiguration();
    HBaseConfiguration.merge(conf, HBaseConfiguration.create(conf));
    ...
    if (quorumAddress != null) {
      // TODO: Calling this will validate the format
    ...
    if (partitioner == HRegionPartitioner.class) {
      ...
      try (Connection conn = ConnectionFactory.createConnection(conf)) {
        regions = conn.getRegionLocator(TableName.valueOf(table)).getAllRegionLocations().size();
      }
