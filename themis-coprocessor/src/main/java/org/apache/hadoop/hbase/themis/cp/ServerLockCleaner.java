...
public class ServerLockCleaner {
  protected final ThemisEndpointClient cpClient;
  protected final Connection conn;

  public ServerLockCleaner(Connection conn, ThemisEndpointClient cpClient) throws IOException {
    this.conn = conn;
  ...
  public void cleanLock(ThemisLock lock) throws IOException {
    ...
    try {
      ...
      Pair<Long, PrimaryLock> cleanResult =
        cleanPrimaryLock(primary.getColumn(), primary.getTimestamp());
      // if the primary lock is cleaned in cleanPrimaryLock, we could get the primary lock with all
      // secondaries
  ...
  // clean primary lock erasing lock. return commitTs if the primary has been committed by other
  // client;
  ...
  public Pair<Long, PrimaryLock> cleanPrimaryLock(ColumnCoordinate columnCoordinate,
    ...
    // commitTs = null indicates the conflicted transaction has been erased by other client;
    // otherwise
  ...
  public Long getTimestampOfWriteIndexingPrewriteTs(ColumnCoordinate columnCoordinate,
  ...
  public Get createGetOfWriteColumnsIndexingPrewriteTs(ColumnCoordinate columnCoordinate,
      long timestamp) throws IOException {
  ...
  public void cleanSecondaryLocks(PrimaryLock primaryLock, Long commitTs) throws IOException {
  ...
  public void eraseLockAndData(ColumnCoordinate column, long timestamp) throws IOException {
    eraseLockAndData(column.getTableName(), column.getRow(), Lists.<Column> newArrayList(column),
      timestamp);
  }

  // erase the lock and data with given timestamp
  public void eraseLockAndData(TableName tableName, byte[] row, Collection<Column> columns,
      long timestamp) throws IOException {
