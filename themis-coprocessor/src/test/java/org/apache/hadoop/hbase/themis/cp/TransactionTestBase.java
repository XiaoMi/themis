...
public class TransactionTestBase extends TestBase {
  ...
  public static void startMiniCluster(Configuration conf) throws Exception {
    ...
    conf.setInt(HConstants.HBASE_CLIENT_RETRIES_NUMBER, 1);
    // timestampBase will increase by 100 each test which will cause the prewriteTs/commitTs is
    // small
  ...
  protected void deleteOldDataAndUpdateTs() throws IOException {
    deleteOldDataAndUpdateTs(getTable(TABLENAME), getTable(ANOTHER_TABLENAME));
  }
    ...
    do {
      ...
      for (Table table : tables) {
        for (byte[] row : new byte[][] { ROW, ANOTHER_ROW, ZZ_ROW }) {
          table.delete(new Delete(row).addFamily(FAMILY, timestampBase)
          ...
          if (!ColumnUtil.isCommitToSameFamily()) {
            table.delete(new Delete(row).addFamily(ColumnUtil.PUT_FAMILY_NAME_BYTES, timestampBase)
              .addFamily(ColumnUtil.DELETE_FAMILY_NAME_BYTES, timestampBase));
          }
        }
        ...
        while ((result = scanner.next()) != null) {
          ...
          System.out.println("###debug, result=" + result + ", table=" + table.getName());
        }
        scanner.close();
      }
  ...
  protected void writeLockAndData(ColumnCoordinate c, long prewriteTs) throws IOException {
    ...
    try (Table table = getTable(c.getTableName())) {
      byte[] lockBytes = ThemisLock.toByte(getLock(c, prewriteTs));
      table.put(
        new Put(c.getRow()).addColumn(lc.getFamily(), lc.getQualifier(), prewriteTs, lockBytes));
    }
  }

  protected void writePutAndData(ColumnCoordinate c, long prewriteTs, long commitTs)
      throws IOException {
    writeData(c, prewriteTs);
    writePutColumn(c, prewriteTs, commitTs);
  }
  ...
  protected void writePutColumn(ColumnCoordinate c, long prewriteTs, long commitTs)
      throws IOException {
    ColumnCoordinate putColumn =
      new ColumnCoordinate(c.getTableName(), c.getRow(), ColumnUtil.getPutColumn(c));
    writeWriteColumnInternal(putColumn, prewriteTs, commitTs);
  }

  protected void writeDeleteColumn(ColumnCoordinate c, long prewriteTs, long commitTs)
      throws IOException {
    ColumnCoordinate deleteColumn =
      new ColumnCoordinate(c.getTableName(), c.getRow(), ColumnUtil.getDeleteColumn(c));
    writeWriteColumnInternal(deleteColumn, prewriteTs, commitTs);
  }

  protected void writeWriteColumn(ColumnCoordinate c, long prewriteTs, long commitTs, boolean isPut)
      throws IOException {
    if (isPut) {
      writePutColumn(c, prewriteTs, commitTs);
    } else {
  ...
  private void writeWriteColumnInternal(ColumnCoordinate c, long prewriteTs, long commitTs)
      throws IOException {
    try (Table table = getTable(c.getTableName())) {
      table.put(new Put(c.getRow()).addColumn(c.getFamily(), c.getQualifier(), commitTs,
  ...
  protected byte[] readLockBytes(ColumnCoordinate c, long prewriteTs) throws IOException {
    ColumnCoordinate lc =
      new ColumnCoordinate(c.getTableName(), c.getRow(), ColumnUtil.getLockColumn(c));
    return readDataValue(lc, prewriteTs);
  }

  protected Long readPut(ColumnCoordinate c) throws IOException {
    return readCommitColumn(
      new ColumnCoordinate(c.getTableName(), c.getRow(), ColumnUtil.getPutColumn(c)));
  }
  ...
  protected byte[] readDataValue(ColumnCoordinate c, long ts) throws IOException {
    ...
    return CellUtil.cloneValue(result.rawCells()[0]);
  }
  ...
  protected void checkCommitAuxiliaryColumnSuccess(ColumnCoordinate auxiliaryCC)
      throws IOException {
    assertArrayEquals(AUXILIARY_VALUE, readDataValue(auxiliaryCC, commitTs));
  }

  // help method for coprocessor write methods
  protected void checkCommitColumnSuccess(ColumnCoordinate c) throws IOException {
    assertNull(readLockBytes(c));
    if (getColumnType(c).equals(Type.Put)) {
      assertArrayEquals(VALUE, readDataValue(c, prewriteTs));
      assertEquals(prewriteTs, readPut(c).longValue());
      assertNull(readDelete(c));
    } else {
      assertNull(readLockBytes(c));
      assertNull(readPut(c));
      assertEquals(prewriteTs, readDelete(c).longValue());
    }
  }
  ...
  protected void checkPrewriteColumnSuccess(ColumnCoordinate c, long prewriteTs, boolean singleRow,
    ...
    assertArrayEquals(lockBytes, readLockBytes(c, prewriteTs));
    if (getColumnType(c).equals(Type.Put) && !singleRow) {
      assertArrayEquals(VALUE, readDataValue(c, prewriteTs));
    } else {
      assertNull(readDataValue(c, prewriteTs));
    }

    if (auxiliaryColumn != null) {
      assertNull(readDataValue(auxiliaryColumn, prewriteTs));
    }
  }

  protected void checkCommitRowSuccess(TableName tableName, RowMutation rowMutation)
      throws IOException {
    for (ColumnMutation mutation : rowMutation.mutationList()) {
      ColumnCoordinate columnCoordinate =
        new ColumnCoordinate(tableName, rowMutation.getRow(), mutation);
      checkCommitColumnSuccess(columnCoordinate);
    }
  }

  protected void checkPrewriteRowSuccess(TableName tableName, RowMutation rowMutation)
      throws IOException {
  ...
  protected void checkPrewriteRowSuccess(TableName tableName, RowMutation rowMutation,
      boolean singleRow, ColumnCoordinate auxiliaryCoordinate) throws IOException {
    for (ColumnMutation mutation : rowMutation.mutationList()) {
      ColumnCoordinate columnCoordinate =
        new ColumnCoordinate(tableName, rowMutation.getRow(), mutation);
  ...
  protected void checkColumnRollback(ColumnCoordinate columnCoordinate) throws IOException {
    ...
    assertNull(readPut(columnCoordinate));
    assertNull(readDelete(columnCoordinate));
  }
  ...
  protected List<ThemisLock> prewriteSecondaryRows() throws IOException {
    ...
    for (Pair<TableName, RowMutation> secondary : SECONDARY_ROWS) {
  ...
  protected void commitSecondaryRow() throws IOException {
    for (int i = 0; i < SECONDARY_ROWS.size(); ++i) {
      TableName tableName = SECONDARY_ROWS.get(i).getFirst();
      RowMutation mutation = SECONDARY_ROWS.get(i).getSecond();
      cpClient.commitSecondaryRow(tableName, mutation.getRow(), mutation.mutationListWithoutValue(),
        prewriteTs, commitTs);
    }
  }
  ...
  protected void truncateTable(TableName tableName) throws IOException {
    try (Admin admin = connection.getAdmin()) {
      admin.disableTable(tableName);
  ...
  protected void checkResultKvColumn(Column expect, Cell kv) {
    ...
    assertEquals(column, new Column(CellUtil.cloneFamily(kv), CellUtil.cloneQualifier(kv)));
  }
}
